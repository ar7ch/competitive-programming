...
int main()
{
  ...
  std::cin >> n;
  std::string crew[101];
  std::string crewStatus[101];
  std::string rats[101];
  int howManyRats = 0;
  int ratsLastPosition = 0;
  std::string womenAndChildren[101];
  int howManyWAC = 0;
  int WACLastPosition = 0;
  std::string men[101];
  int howManyMen = 0;
  int menLastPosition = 0;
  std::string captain = "no_captain";

  for(int i = 0; i < n; i++){
    std::cin >> crew[i];
    std::cin >> crewStatus[i];
    if(crewStatus[i] == "rat"){
      rats[ratsLastPosition] = crew[i];
      ratsLastPosition++;
      howManyRats++;
      continue;
    }
    else if(crewStatus[i] == "woman" || crewStatus[i] == "child"){
      womenAndChildren[WACLastPosition] = crew[i];
      WACLastPosition++;
      howManyWAC++;
      continue;
    }
    else if(crewStatus[i] == "man"){
      men[menLastPosition] = crew[i];
      menLastPosition++;
      howManyMen++;
      continue;
    }
    else if( crewStatus[i] == "captain"){
      captain = crew[i];
      continue;
    }
  }
  ...
  if(howManyWAC > 0){
  for(int i = 0; i < howManyWAC; i++)
    std::cout << womenAndChildren[i] << '\n';
  }
  if(howManyMen > 0){
  for(int i = 0; i < howManyMen; i++)
    std::cout << men[i] << '\n';
  }
  if(captain != "no_captain")
    std::cout << captain << '\n';
  return 0;
}
